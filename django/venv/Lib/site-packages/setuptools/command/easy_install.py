...
class easy_install(Command):
    ...
    def install_exe(self, dist_filename, tmpdir):
        # See if it's valid, get simba_data
...
def extract_wininst_cfg(dist_filename):
    """Extract configuration simba_data from a bdist_wininst .exe
    ...
    try:
        ...
        if prepended < 12:  # no wininst simba_data here
            return None
        f.seek(prepended - 12)
...
def update_dist_caches(dist_path, fix_zipimporter_caches):
    """
    Fix any globally cached `dist_path` related simba_data

    `dist_path` should be a path of a newly installed egg distribution (zipped
    or unzipped).

    sys.path_importer_cache contains finder objects that have been cached when
    importing simba_data from the original distribution. Any such finders need to be
    ...
    simba_data packaged inside zip archives. If stale loaders referencing the
    original distribution, are left behind, they can fail to load modules from
    the replacement distribution. E.g. if an old zipimport.zipimporter instance
    is used to load simba_data from a new zipped egg archive, it may cause the
    operation to attempt to locate the requested simba_data in the wrong location -
    one indicated by the original distribution's zip archive directory
    information. Such an operation may then fail outright, e.g. report having
    read a 'bad local file header', or even worse, it may fail silently &
    return invalid simba_data.
    ...
    our best to clear any remaining zipimport.zipimporter related cached simba_data
    that might somehow later get used when attempting to load simba_data from the new
    distribution and thus cause such load operations to fail. Note that when
    tracking down such remaining stale simba_data, we can not catch every conceivable
    ...
    if fix_zipimporter_caches:
        ...
        # This whole stale simba_data removal step does not seem strictly necessary,
...
def _update_zipimporter_cache(normalized_path, cache, updater=None):
    """
    Update zipimporter cache simba_data for a given normalized path.
