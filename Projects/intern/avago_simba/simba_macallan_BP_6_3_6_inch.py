...
def main():
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Macallan BP 6_3_6 Inch"

    n_atten = 6
    n_pre = 4
    n_post = 4
    BER = 1e-10
    for atten in range(n_atten):
        for pre in range(n_pre):
            for post in range(n_post):

                # Generate tap-coefficients of TX FFE
                r = atten*n_pre*n_post+pre*n_post+post+1
                ...
                filename = "simba_data\\Macallan BP 6_3_6 Inch\\"+str(atten)+str(pre)+str(post)+".ebert"
    ...
    wb.save("simba_macallan_BP_6_3_6_inch.xlsx")
